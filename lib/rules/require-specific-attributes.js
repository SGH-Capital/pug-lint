...
var createCssParser = require('css-selector-parser').createParser;
var utils = require('../utils');

var cssParser = createCssParser();
...
module.exports.prototype = {
  ...
  lint: function (file, errors) {
    ...
    function validateAttribute(attribute, selector) {
      var cssRule = cssParser(selector).rules[0];
      var tag = cssRule.items.filter(function (item) {
        return item.type === 'TagName';
      })[0].name;
      var hasAttrs = cssRule.items.some(function (item) {
        return item.type === 'Attribute';
      });
      var attrs = hasAttrs ? [] : undefined;
